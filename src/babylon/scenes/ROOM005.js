...
export const build = ({scene}) => {
  ...
  // const blackColor = BABYLON.Color3.FromHexString('#ccf');
  const whiteColor = new BABYLON.Color3(16 / 255, 22 / 255, 26 / 255);
  const blackColor = new BABYLON.Color3(245 / 255, 248 / 255, 250 / 255);
  ...
  fillerMesh.thinInstanceSetBuffer('matrix', bufferMatrices, 10);
  ...
  [fillerMaterial, contourMaterial].forEach((material) => {
    ...
    material.onBind = () => {
      ...
      material.getEffect().setFloat('swingSize', Math.sin(time * 0.001));
    };
  });
